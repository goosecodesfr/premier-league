...
import type { Instructions, TacticSlot } from '../tactics.ts';
...
export function attackPresence(p: PresenceParams, out: Float64Array) {
  ...
  const sb = Math.max(0.4, 0.8 * r.roam);
...
export function defencePresence(p: PresenceParams, out: Float64Array) {
  ...
  const sb = Math.max(0.4, 0.75 * r.roam);
  fill(out, band, chan, sb, 0.55, p.activity);
}
